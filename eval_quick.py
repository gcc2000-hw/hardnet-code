...
# Import logging for VEG optimization logging
import logging
...
class BetaSpring10KEvaluator:
    ...
    def __init__(self):
        print("=" * 60)
        print("BETA SPRING 10K EVALUATION - SIMPLIFIED")
        print("=" * 60)
        
        # Create output directories for Beta SPRING
        self.output_dir = Path('beta_evaluation_10k_results')
    ...
    def get_completed_samples(self):
        ...
        if self.images_dir.exists():
            for image_file in self.images_dir.glob("*_generated.png"):
                # Extract sample ID from filename (e.g., "kitchen_0049_generated.png" -> "kitchen_0049")
                sample_id = image_file.stem.replace("_generated", "")
    ...
    def load_evaluation_data(self):
        ...
        try:
            # Load specifications
            specs_path = 'data/evaluation/specifications_realistic_cleaned_FIXED.json'
            ...
            detections_path = 'data/evaluation/background_detections.json'
    ...
    def process_single_sample(self, spec: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            background_path = f"data/evaluation/backgrounds/{sample_id}.png"
            ...
            # Compile simplified results - ONLY constraint satisfaction and time
            sample_result = {
                'id': sample_id,
                ...
                'final_coordinates': merged_layout_tensor
            }
            
            print(f"{sample_id} completed:")
            print(f"  Time: {generation_time:.2f}s")
            print(f"  Constraint satisfaction: {constraint_results['satisfaction_rate']:.1%}")
    ...
    def run_evaluation(self, max_samples: int = None, start_idx: int = 0):
        ...
        try:
            print(f"\nStarting simplified 10K evaluation...")
    ...
    def save_final_results(self):
        """Save final evaluation results with simplified analysis and VEG image mappings."""
        try:
            ...
            # Compute simplified aggregate statistics - ONLY time and constraint satisfaction
            stats = {
                ...
                # Performance metrics - ONLY time and constraint satisfaction
                ...
                # VEG Image Generation Summary
                'image_generation_summary': {
                    'total_veg_images_generated': len([r for r in successful_results if 'veg_generated_image' in r]),
                    'total_annotated_images': len([r for r in successful_results if 'annotated_image' in r]),
            ...
            # Create VEG image mapping for easy lookup
            veg_image_mapping = {}
            for result in successful_results:
                sample_id = result['id']
                veg_image_mapping[sample_id] = {
                    'constraint_satisfaction_rate': result['constraint_satisfaction']['satisfaction_rate'],
                    'generation_time': result['time'],
                    'veg_generated_image': result.get('veg_generated_image', ''),
                    'annotated_image': result.get('annotated_image', ''),
                    'background_image': result.get('background_image', ''),
                    'generated_objects': result.get('generated_objects', []),
                    'existing_objects': result.get('existing_objects', [])
                }
            
            final_results = {
                'evaluation_summary': stats,
                'veg_image_mapping': veg_image_mapping,  # Quick lookup: sample_id -> images + metrics
            ...
            # Save a simplified VEG mapping file for easy access
            veg_mapping_file = self.output_dir / 'veg_image_constraint_mapping.json'
            with open(veg_mapping_file, 'w') as f:
                json.dump({
                    'description': 'Mapping of VEG-generated images to constraint satisfaction scores',
                    'image_directory': str(self.images_dir),
                    'mapping': veg_image_mapping
            ...
            print(f"VEG images generated: {stats['image_generation_summary']['total_veg_images_generated']}")
            print(f"\nResults saved to: {final_file}")
            print(f"VEG mapping saved to: {veg_mapping_file}")
            print(f"Generated images in: {self.images_dir}")
            
            # Print some example VEG mappings
            print(f"\nExample VEG Image Mappings:")
            for i, (sample_id, mapping) in enumerate(list(veg_image_mapping.items())[:3]):
                print(f"  {sample_id}:")
                print(f"    Constraint satisfaction: {mapping['constraint_satisfaction_rate']:.1%}")
                print(f"    VEG image: {Path(mapping['veg_generated_image']).name}")
                print(f"    Generated objects: {mapping['generated_objects']}")
    ...
    def _compute_room_type_stats(self, results: List[Dict]) -> Dict:
        """Compute simplified statistics broken down by room type."""
        room_stats = {}
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Simplified SPRING 10K Evaluation')
    ...
    try:
        ...
        print(f"Starting simplified evaluation: start_sample={args.start_sample}, num_samples={args.num_samples}")
